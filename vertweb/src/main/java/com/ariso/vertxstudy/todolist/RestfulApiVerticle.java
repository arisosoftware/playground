...
/**
 * An abstract base reactive verticle that provides several helper methods for
 * RESTful API.
 *
...
public abstract class RestfulApiVerticle extends AbstractVerticle {
    ...
    /**
     * Resolve an asynchronous status and send back the response. By default, the
     * successful status code is 200 OK.
     *
     * @param context
     *            routing context
     * @param asyncResult
     *            asynchronous status with no result
     */
    ...
    /**
     * Resolve an asynchronous status and send back the response. The successful
     * status code depends on processor {@code f}.
     *
     * @param context
     *            routing context
     * @param asyncResult
    ...
    protected <T> void sendResponse(RoutingContext context, Maybe<T> asyncResult, Function<T, String> converter) {
        if (asyncResult == null) {
            ...
            Single<Optional<T>> single = asyncResult.map(Optional::of).switchIfEmpty(Maybe.just(Optional.empty()))
                    .toSingle();
